...
@section('header')
    <section class="content-header">
        <h1>
            ...
            <small>{{ trans('backpack::crud.all') }} <span class="text-lowercase">{{ $crud->entity_name_plural }}</span> con las que usted se relaciona.</small>
        </h1>
        <ol class="breadcrumb">
            <li><a href="{{ url(config('backpack.base.route_prefix'), '') }}">Promotor</a></li>
                        ...
                        @if (!$crud->ajaxTable())
                            @foreach ($entries as $k => $entry)
                                <tr data-entry-id="{{ $entry->getKey() }}">
                                    ...
                                    @endforeach
                                    <td>{{$entry->calle}} No. {{$entry->noExterior}} COL. {{$entry->colonia}}, {{$entry->municipio}}, {{$entry->estado}}, {{$entry->pais}} C.P. {{$entry->codigoPostal}}</td>
